...
public class ClassUtil {
    ...
    /**
     * 通过接口名取得某个接口下所有实现这个接口的非抽象类
     */
    public static List<Class<?>> getConcreteClassByInterface(Class<?> c) {
        List<Class<?>> returnClassList = null;
        ...
        if (c.isInterface()) {
            ...
            if (allClass != null) {
                ...
                for (Class<?> cls : allClass) {
                    ...
                    if (c.isAssignableFrom(cls)) {
                        // 本身以及抽象类不加入进去
                        if (!c.equals(cls) && !Modifier.isAbstract(cls.getModifiers())) {
                            returnClassList.add(cls);
                        }
                    }
                }
     ...
     * 取得某个包名下所有具有某个注解的类
     */
    public static List<Class<?>> getAllClassByAnnotation(String packageName, Class<? extends Annotation> annotationCls) {
        List<Class<?>> returnClassList = null;
